...
export function Footer() {
  return (
    <footer className="relative overflow-hidden pt-[clamp(60px,7vw,100px)] pb-10 bg-oria-black text-text-light">
      ...
      >
        MAISON <em style={{ fontStyle: "italic", WebkitTextStroke: "1px var(--gold-deep)" }}>ORIA</em>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-15 px-[clamp(20px,4vw,60px)] mb-15">
        <div>
          <p className="mb-8 max-w-[440px] font-serif italic text-[clamp(22px,2vw,30px)] leading-[1.4] text-text-light">
        ...
        <div>
          ...
          <form
            ...
            onSubmit={(e) => { e.preventDefault(); e.currentTarget.querySelector("button").textContent = "Welcome ✦"; }}
          >
            <input
              type="email"
              required
      ...
      <div className="grid md:grid-cols-3 gap-6 px-[clamp(20px,4vw,60px)] pt-8 items-center text-[11px] tracking-[0.18em] uppercase border-t border-[rgba(245,240,234,0.08)] text-[rgba(245,240,234,0.5)]">
        ...
        <div className="flex gap-[18px] md:justify-center flex-wrap">
        ...
        <div className="flex md:justify-end gap-2.5 flex-wrap">
          {["VISA", "MC", "AMEX", "PAYPAL", "COD"].map((m) => (
            <span
              key={m}
              className="border border-[rgba(245,240,234,0.18)] rounded px-2.5 py-1 text-[9px] font-semibold text-[rgba(245,240,234,0.7)]"
            >
              {m}
            </span>
          ))}
